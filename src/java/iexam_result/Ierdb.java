    ...
    public class Ierdb {   
        ...
        public static int update(Ier ier){  
            ...
            try{  
                Connection con=dbconnection.getConnection();  
                PreparedStatement ps=con.prepareStatement(  
                             "update individual_exam_result set ierindex_no=?,iergrade=?,iersubject=?,ierresult=? where ierid=?");  
                ps.setString(1,ier.getIerindex_no());  
                ps.setString(2,ier.getIergrade());  
                ps.setString(3,ier.getIersubject());  
                ps.setString(4,ier.getIerresult());  
        ...
        public static Ier getIEResultByIerid(int ierid){  
            ...
            try{  
                Connection con=dbconnection.getConnection();  
                PreparedStatement ps=con.prepareStatement("select * from individual_exam_result where ierid=?");  
                ps.setInt(1,ierid);  
